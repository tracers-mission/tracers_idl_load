...
;
;-
pro tracers_roi_times_load, remote_path = remote_path, local_path = local_path, $
  ...
  for i = 0, ndates - 1 do begin
    ...
    roi_path = '/flight/SOC/' + strupcase(spacecraft) + '/events/roi_intervals/'
    fn_i = roi_path + strlowcase(spacecraft) + '_roi-list.csv'
    ...
    ; data = read_ascii(data_filenames[0], delimiter = ',', data_start=7) ; read in the csv file, skipping the header rows

    data = read_ascii(data_filenames[0], template = roi_template)
    stop
  endfor ; dates
end

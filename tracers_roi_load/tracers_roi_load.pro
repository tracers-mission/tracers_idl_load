...
;
; :Returns:
;   Struct {tstart: str_arr, tend: str_arr} where times are time_string() format,
;   or [] if no overlapping intervals found
;
; :Notes:
...
;   roi_out: out, optional, Struct
;     Returned structure with one sub-struct per spacecraft. Each sub-struct
;     contains parallel string arrays of start and end times. Access via:
...
;     If set, downloads CSV file(s) but does not parse or populate roi_out.
;
; :Examples:
;   timespan, '2025-11-08', 3
;   tracers_roi_load, roi_out = rois
;   print, rois.ts1.tstart              ; all TS1 start times in the window
;   print, rois.ts2.tend                ; all TS2 end times in the window
;
;   tracers_roi_load, spacecraft = 'ts1', trange = ['2025-11-08', '2025-11-12'], roi_out = rois
;   n = n_elements(rois.ts1.tstart)
;   for i = 0, n - 1 do print, rois.ts1.tstart[i], ' -- ', rois.ts1.tend[i]
;
;   tracers_roi_load, spacecraft = 'tandem', roi_out = rois
;   print, rois.tandem.tstart
;
;   tracers_roi_load, spacecraft = ['ts1', 'ts2', 'tandem'], roi_out = rois
;   tracers_roi_load, spacecraft = ['both', 'tandem'], roi_out = rois
;
; :Created by:
;   Sky Shaver    Apr 2026
;
...
;
;-
pro tracers_roi_load, spacecraft = spacecraft, trange = trange, $
  local_path = local_path, remote_path = remote_path, $
  roi_out = roi_out, downloadonly = downloadonly
  compile_opt idl2

  if undefined(spacecraft)  then spacecraft  = ['both'] else spacecraft = strlowcase(spacecraft)
  ...
  do_ts1    = max(spacecraft eq 'ts1') or max(spacecraft eq 'both')
  do_ts2    = max(spacecraft eq 'ts2') or max(spacecraft eq 'both')
  do_tandem = max(spacecraft eq 'tandem')

  if ~do_ts1 and ~do_ts2 and ~do_tandem then begin
    dprint, "Invalid spacecraft keyword ['" + strjoin(spacecraft, "', '") + "']. Use 'ts1', 'ts2', 'both', 'tandem', or an array combination."
    return
  endif
